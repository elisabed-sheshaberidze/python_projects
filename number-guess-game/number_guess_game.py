        ...
        while True:
            try:
                guessed_number = input(
                    f'Guess the number between {LOWEST_NUMBER} and {maximum_number}.'
                    f' Psst... The number is {hint} 😉. ')
                ...
                if int(guessed_number) == number_to_guess:
                    total_points += award
                    print(
                        f'Correct! You earned {award} points! Your total points: {total_points}')
                    break
                    ...
                    total_points -= deduct
                    print(
                        f'Too low! You lost {deduct} points! Your total points: {total_points}')
                elif int(guessed_number) > number_to_guess:
                    
                    total_tries -= 1
                    total_points -= deduct
                    print(
                        f'Too high! You lost {deduct} points! Your total points: {total_points}')
                if total_tries == 0:
                    print(
                        f'No more tries left! The correct number was - "{number_to_guess}".')
...
def rules(user):
    ...
    print(f'''Welcome {user} to the Number Guessing Game!
          ...
          • Rewards: easy mode +8 points, medium mode +15 points, hard mode +25 points.
...
def main():
    ...
    if maximum_number == None:
        return print('You have quitted the game! Goodbye! 👋')
